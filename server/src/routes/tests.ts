...
router.post("/", protect, uploadTestImage.fields([{ name: "img", maxCount: 1 }, { name: "questionImgs", maxCount: 20 }]), createTest);
router.get("/:id", protect, getTestById);
router.patch("/:id", protect, uploadTestImage.fields([{ name: "img", maxCount: 1 }, { name: "questionImgs", maxCount: 20 }]), updateTest);
