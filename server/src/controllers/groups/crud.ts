...
export const createGroup = async (req: Request, res: Response): Promise<void> => {
  ...
  const avatar = req.file ? `/uploads/groups/${req.file.filename}` : "";

  const group = await Group.create({ name, description, avatar, admin: userId, members: [userId] });
...
export const updateGroup = async (req: Request, res: Response): Promise<void> => {
  ...
  const updated = await Group.findByIdAndUpdate(req.params.id, { name, description, avatar }, { new: true });
  res.json(updated);
};
...
export const getMyGroups = async (req: Request, res: Response): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) { res.status(401).json({ message: "Не авторизован" }); return; }
  const groups = await Group.find({ members: userId }).populate("admin", "username avatar").sort({ createdAt: -1 });
  res.json(groups);
};