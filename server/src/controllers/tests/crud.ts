...
export const createTest = async (req: Request, res: Response): Promise<void> => {
  ...
  const indexes = Array.isArray(req.body.questionImgIndexes) ? req.body.questionImgIndexes : req.body.questionImgIndexes ? [req.body.questionImgIndexes] : [];
  questionImgs.forEach((file, i) => {
    const idx = parseInt(indexes[i], 10);
    if (!isNaN(idx) && parsedQuestions[idx]) parsedQuestions[idx].img = `/uploads/tests/${file.filename}`;
  });

  const test = await Test.create({ title, img, description, author: userId, question: parsedQuestions });
...
export const updateTest = async (req: Request, res: Response): Promise<void> => {
  ...
  const img = files?.img?.[0] ? `/uploads/tests/${files.img[0].filename}` : req.body.img || test.img;
  ...
  const updated = await Test.findByIdAndUpdate(req.params.id, { title, img, description, question: parsedQuestions }, { new: true });
  res.json(updated);
};
